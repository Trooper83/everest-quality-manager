...
function addEnvTag() {
    ...
    if(text.trim().length > 0) {
        ...
        let pill = $(`<h3><p class="font-weight-normal badge badge-light border border-secondary border-rounded">${text}
                <svg xmlns="http://www.w3.org/2000/svg" style="cursor:pointer" width="12" height="12" fill="currentColor" class="bi bi-pencil ml-1" viewBox="0 0 20 20" onclick="displayEnvOptions(this)">
...
*/
function displayEnvOptions(element, id) {
    ...
    let buttonGroup = $('<div class="btn-group btn-group-sm" role="group" aria-label="edit buttons">' +
        '<button class="btn btn-link" data-test-id="remove-tag-button" type="button" onclick="removeEnvTag(this,' + id + ')">remove</button>' +
        '<button class="btn btn-link" data-test-id="edit-tag-button" type="button" onclick="editEnvTag(this,' + id + ')">edit</button>' +
        '</div>');
...
function editEnvTag(button, id) {
    ...
    ele.append('<input class="btn btn-primary btn-sm border" type="button" data-test-id="save-tag-button" value="Save" onclick="saveEnvTag(this,' + id + ')"/>');
    ele.append('<input class="btn btn-light btn-sm border" type="button" data-test-id="cancel-tag-button" value="Cancel" onclick="cancelEnvTag(this,' + id + ')"/>');
    ...
    $(document).on('click', function(event) {
        ...
        if(!ele.is(event.target)) {
            ...
            ele.val(text);
            let pill = $('<h3><p class="font-weight-normal badge badge-light border border-secondary border-rounded">' + text +
                                  '<svg xmlns="http://www.w3.org/2000/svg" style="cursor:pointer" width="12" height="12" fill="currentColor" class="bi bi-pencil ml-1" viewBox="0 0 20 20" onclick="displayEnvOptions(this,' + id + ')">' +
                                  '<path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>' +
                                  '/svg></p></h3>');
...
* cancels a tag
*/
function cancelEnvTag(element, id) {
    let ele = $(element);
...
* saves a tag
*/
function saveEnvTag(element, id) {
    let ele = $(element);
    let input = ele.siblings('input[type="text"]');
    let text = input.val();
    if(text.trim().length > 0) {
